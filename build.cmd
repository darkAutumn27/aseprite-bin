...
where /q ninja.exe || (
  ...
  %SZIP% x -bb0 -y ninja-win.zip >nul || exit /b 1
  del ninja-win.zip >nul
)

rem *** latest Aseprite version ***

set ASEPRITE_VERSION=v1.14.1
...
if exist aseprite (
  ...
  git fetch --quiet --depth=1 --no-tags origin %ASEPRITE_VERSION%:refs/remotes/origin/%ASEPRITE_VERSION% || echo "failed to fetch latest version" && exit /b 1
  git reset --quiet --hard origin/%ASEPRITE_VERSION% || echo "failed to update to latest version" && exit /b 1
...
if not exist skia-%SKIA_VERSION% (
  ...
  %SZIP% x -y Skia-Windows-Release-x64.zip >nul || exit /b 1
  popd
)
...
cmake.exe                                                     ^
  ...
  -DSKIA_OPENGL_LIBRARY=opengl32.lib                          || echo failed to configure build && exit /b 1
